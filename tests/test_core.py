...
def test_policy_enumeration_and_verification():
    s = Scenario(
        ...
        {"capacity": 0},
        {"a": (0, 1), "b": (0, 1)},
        lambda st, p: {"capacity": 10 * p["a"] + 5 * p["b"]},
        (Constraint("capacity", lambda x: x["capacity"] >= 10, "capacity too low"),),
    )
...
def test_robust_engine_runs_quantum_and_classical_paths():
    ...
    assert len(result.adversarial_verification) == 3
    assert result.verification.passed == result.adversarial_verification[0].passed
