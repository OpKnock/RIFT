...
- Product boundaries: `experiments.py` (specs/fingerprints), `limits.py` (bounds), `auth.py` (token gate + ownership), `settings.py` (server env), `supabase_store.py`/`persistence.py` (DB), `billing.py` (Lemon Squeezy), `observability.py` (request IDs/logs), `api.py` (HTTP), `cli.py`, `qpu.py` (gated hardware stub)
- Migrations (source of truth): `backend/supabase/migrations/` 001→004
- Frontend: `web/` (lab + status/history/settings panels)
- Tests: `tests/` (unit + API boundary + auth + billing lifecycle + hardening)
