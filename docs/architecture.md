...
- rift.health: patient twin demo (EHR/wearable sources/baseline/transition/risk/twin/FORESIGHT/robustness/Guardian/explain/evaluate); v1 platform: canonical observations + FHIR/CSV/JSON adapters + timeline estimation, model registry with weights pinning, deterministic prediction provenance, decision-table comparison, deployment gate; adapts PatientState to the generic Scenario — engine modules stay domain-free
