...
pytest -q   # 221 passing on the release commit
```
...
1. Server won't start → `python -m rift.cli twin-demo` (full twin story in terminal) + `pytest -q` (221 green) + `docs/evidence-sheet.md` (frozen numbers).
