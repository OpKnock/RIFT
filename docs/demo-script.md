...
pytest -q   # 281 passing on the release commit
```
...
1. Server won't start → `python -m rift.cli twin-demo` (full twin story in terminal) + `pytest -q` (281 green) + `docs/evidence-sheet.md` (frozen numbers).
