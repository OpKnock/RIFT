...
def assess_candidate(scenario: Scenario, candidate: Future, perturbations: list[dict[str, float]]) -> RobustAssessment:
    ...
    if not failures:
        return RobustAssessment(candidate, None, candidate.score, 0.0)
    worst = failures[0]
    mean = sum(x.adversarial_score for x in failures) / len(failures)
    return RobustAssessment(candidate, worst, mean, worst.adversarial_score - candidate.score)
...
def rank_robust_candidates(scenario: Scenario, candidates: list[Future], perturbations: list[dict[str, float]]) -> list[RobustAssessment]:
    ...
    return sorted(assessments, key=lambda x: (x.worst_case.adversarial_score if x.worst_case else x.candidate.score, x.mean_adversarial_score))
