...
class ExperimentResult:
    ...
    adversarial_verification: tuple[VerificationResult, ...] = ()
...
def run_experiment(
    scenario: Scenario,
    qubo: QUBO,
    perturbations: list[dict[str, float]] | None = None,
) -> ExperimentResult:
    candidates = tuple(generate_futures(scenario))
    perturbations = perturbations or []
    ranked = (
        tuple(rank_robust_candidates(scenario, list(candidates), perturbations))
        if perturbations
        else ()
    )
    robust_qubo = (
        build_robust_qubo(scenario, qubo.variables, perturbations)
        if perturbations
        else None
    )
    if robust_qubo:
    ...
    adversarial = verify_under_perturbations(
